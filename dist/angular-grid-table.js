/*!
 * angular-grid-table
 * @version: 0.0.1 - 2016-06-06T10:16:01.885Z
...
$templateCache.put("grid-table-items.html","<tr ng-init=\"itemsIndex = $index + 1\" ng-repeat=\"item in $grid.getItems()\" ng-click=\"$grid.rowClick(item, $event)\" ng-dblclick=\"$grid.rowDblClick(item, $event)\" ng-class=\"{\'selectable\': $grid.selectable, \'active\': $grid.isItemSelected(item)}\" class=\"grid-table-item\"><td ng-repeat=\"column in $grid.getShowColumns()\">{{column.columnType == \'numbers\' ? itemsIndex : \'\'}} <span ng-if=\"column.columnType == \'checkbox\'\"><input ng-click=\"$grid.itemSelect(item)\" ng-checked=\"$grid.isItemSelected(item)\" type=\"checkbox\"></span> <span ng-if=\"$grid.isLinkColumn(column.name)\"><a ng-click=\"$grid.rowColumnClick(item, column, $event)\" href=\"#\">{{$grid.getValue(item, column.name) | gridTableFormatter:column.type}}</a></span> <span ng-if=\"!$grid.isLinkColumn(column.name)\">{{$grid.getValue(item, column.name) | gridTableFormatter:column.type}}</span> <span ng-if=\"column.columnType === \'actions\' && $grid.itemActions && $grid.itemActionsShow(item)\"><span ng-repeat=\"action in $grid.itemActions\" ng-click=\"$grid.itemActionCall(action.fn, item, $event)\"><span ng-if=\"eval(action.exp)\"><span grid-table-item-action=\"\" html=\"action.html\"></span></span></span></span></td></tr><tr><td ng-show=\"$grid.itemsCount <= 0\" colspan=\"{{$grid.columnsCount}}\">{{$grid.text.empty}}</td></tr>");
$templateCache.put("grid-table-toolbar.html","<div class=\"grid-table-tools\"><span class=\"tools-label\">{{$grid.text.tools}}</span><ul class=\"tools\"><li><a ng-click=\"$grid.refresh($event)\" href=\"#\" title=\"{{$grid.text.refreshTitle}}\">{{$grid.text.refresh}}</a></li><li><a ng-click=\"$grid.clearFilter($event)\" href=\"#\" title=\"{{$grid.text.clearFilterTitle}}\">{{$grid.text.clearFilter}}</a></li></ul></div><div class=\"grid-table-pager\"><span class=\"pager-label\">{{$grid.text.pager}}</span><ul class=\"pager\"><li ng-repeat=\"page in $grid.pager.items\" ng-click=\"$grid.setPage(page.index, $event)\" ng-class=\"{\'active\': page.index == $grid.getPage()}\" ng-disabled=\"page.disable\"><a href=\"#\">{{page.label}}</a></li></ul></div><div class=\"grid-table-view-by\"><span class=\"view-by-label\">{{$grid.text.viewBy}}</span><ul class=\"view-by\"><li ng-repeat=\"item in $grid.viewByList\" ng-click=\"$grid.setViewBy(item, $event)\" ng-class=\"{\'active\': item == $grid.viewBy}\"><a href=\"#\">{{item}}</a></li></ul></div><div class=\"grid-table-clear\"></div>");
